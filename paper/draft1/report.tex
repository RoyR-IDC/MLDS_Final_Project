...
\twocolumn[{
\centering
\includegraphics[width=0.3\textwidth]{images/runi_logo.png}\\[0.5em]
{\LARGE Reichman University}\\[0.75em]
{\large Efi Arazi School of Computer Science}\\[0.75em]
{\large M.Sc. Program in Machine Learning and Data Science}\\[0.75em]
\vspace{0.75em}
{\Huge Final Project Report}\\[1em]
{\Large Permuted Dogs versus Cats}\\[0.75em]
{\small \textbf{Student 1} \quad \texttt{ID: 123456789} \quad \textbf{Student 2} \quad \texttt{ID: 987654321}}\\[0.75em]
{\small \textbf{Advisors:} Shuli Finley (Reichman University) \space --- \space Alon Oring (Reichman University)}\\[0.75em]
{\small \today}\\[0.75em]
\begin{flushleft}
\begin{abstract}
Natural-image classifiers usually benefit from coherent spatial layout, yet class evidence may survive even when that layout is systematically disrupted. This report studies binary Dogs-versus-Cats classification under fixed tile-wise image permutations. The task is motivated by structured-permutation hardness questions, but is evaluated here as an empirical computer-vision benchmark rather than as a cryptographic claim. Images from the Kaggle Dogs vs Cats dataset are divided into square grids and transformed by deterministic tile rearrangements across four grid settings and three permutation presets. The experiments compare frozen pretrained ResNet-18, DeiT-Tiny, and MLP-Mixer backbones with trained binary classification heads; evaluate ResNet-18 ablations involving augmentation, mixed-input training, classifier-head capacity, and curriculum strategies; and compute model-agnostic permutation hardness metrics joined to validation accuracy. Performance remains well above chance across the deterministic matrix, but degrades as the grid becomes finer and the permutation preset becomes more disruptive. DeiT-Tiny is strongest in the frozen-backbone comparison, the ResNet-18 MLP head gives the largest average ablation improvement among the tested strategies, and adjacency destruction has the strongest negative association with ResNet-18 best validation accuracy among the tested hardness metrics. The findings are limited to fixed deterministic permutations and frozen pretrained representations, and should be interpreted as evidence that useful class cues survive the transformations rather than as evidence that the models reconstruct or invert the original layout.
\end{abstract}
\end{flushleft}
\vspace{0em}
}]
...
\begin{figure}[t]
  ...
  \begin{tabular}{>{\centering\arraybackslash}p{0.18\linewidth}
    ...
    &
    \fbox{\begin{minipage}{0.80\linewidth}
      \centering Residual\\stages\\64--512 channels
    \end{minipage}}
    &
    \fbox{\begin{minipage}{0.80\linewidth}
    ...
    &
    \fbox{\begin{minipage}{0.80\linewidth}
      \centering Trainable\\binary\\head
    \end{minipage}}
    \\
