...
\begin{table*}[t]
  ...
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{p{0.09\textwidth}p{0.17\textwidth}p{0.22\textwidth}p{0.24\textwidth}p{0.24\textwidth}}
    \toprule
    \textbf{Part} & \textbf{Models or analysis} & \textbf{Permutations / settings} & \textbf{Training set} & \textbf{Validation set} \\
    \midrule
    Part 1 & ResNet-18, DeiT-Tiny, MLP-Mixer Base & Unpermuted baseline with \texttt{tile\_permutation\_id=0}; five 4-by-4 permutations; five 10-by-10 permutations & Fixed 20,000-image training split, transformed with the row's target image size and tile permutation. The unpermuted baseline uses the same images without tile reordering. & Fixed 5,000-image validation split, transformed with the same target permutation as the run. \\
    Part 2 & ResNet-18 ablations & Patch shuffle, random erasing, same-label CutMix, focal loss, corruption-probability curriculum, and permutation-difficulty curriculum; baseline, three 4-by-4 permutations, and three 10-by-10 permutations where available & Fixed 20,000-image training split with the ablation's training transform, batch augmentation, loss, or curriculum schedule applied. & Fixed 5,000-image validation split with the target run permutation only; training-only augmentations are disabled. \\
    Part 3 & Hardness analysis, no new model training & Reuses Part 1 ResNet-18 tile-permutation records and validation accuracies & Not applicable; Part 3 computes permutation metrics and does not update model parameters. & Part 1 ResNet-18 held-out validation accuracies are joined to the corresponding permutation metrics. \\
    \bottomrule
  \end{tabular}
  }
  \caption{Training and validation sets used by each experiment part in the current output matrix. Full permutation matrices are stored in the saved permutation metadata rather than repeated in the report.}
  \label{tab:train-validation-by-experiment}
\end{table*}

The six Part 2 ablation identifiers in the saved results are \texttt{augmentation\_\allowbreak patch\_\allowbreak shuffle}, \texttt{augmentation\_\allowbreak random\_\allowbreak erasing}, \texttt{augmentation\_\allowbreak same\_\allowbreak label\_\allowbreak cutmix}, \texttt{loss\_\allowbreak focal\_\allowbreak loss}, \texttt{curriculum\_\allowbreak corruption\_\allowbreak probability}, and \texttt{curriculum\_\allowbreak permutation\_\allowbreak difficulty}.

The curriculum ablations in Part 2 use additional stage-specific training loaders, but they do not create separate held-out validation splits per stage. In the corruption-probability curriculum, every stage uses the same 20,000 training images and the target tile permutation, with tile-permutation probabilities 0.1, 0.3, 0.5, and 0.8. In the permutation-difficulty curriculum, every stage again uses the same training images, progressing from original images to 2-by-2, 3-by-3, and 4-by-4 permutations, with an additional 10-by-10 stage for 10-by-10 target runs. Across both curricula, validation remains the same 5,000-image held-out loader for the final target run.
