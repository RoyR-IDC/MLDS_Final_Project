...
Table~\ref{tab:part2-results} reports the MobileNetV3-Small CNN ablation summary over the 12 deterministic records, and Figure~\ref{fig:part2-ablation} shows each strategy's delta from its matched baseline.
...
\begin{table*}[t]
  ...
  \caption{Part 2 MobileNetV3-Small CNN ablation summary over the 12 deterministic records. ``Mean final acc.'' is the validation accuracy at the final epoch of the corresponding strategy. ``Mean best acc.'' is the best validation accuracy reached during that same strategy's training run.}
  \label{tab:part2-results}
\end{table*}

The Part 2 matrix is designed to compare each ablation against the matched regular baseline at the same grid size and deterministic difficulty label. The reported deltas should be interpreted within the limited deterministic matrix and the shared validation-set selection protocol.
...
\begin{figure}[t]
  ...
  \begin{minipage}{0.92\linewidth}
  \centering
  \textbf{Part 2 MobileNetV3-Small ablation deltas}\\[0.5em]
  Regular baseline \quad Patch shuffle \quad Regular augmentation\\
  Mixed original/permuted \quad CNN MLP head\\
  Corruption-probability curriculum \quad Permutation-difficulty curriculum\\[0.5em]
  Values are reported as best-validation-accuracy deltas from the matched regular baseline.
  \end{minipage}
  }
